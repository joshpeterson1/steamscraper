...
for i in $(seq 0 4); do
    ...
    ffmpeg -i "$input_file" -vf "crop=${strip_width}:in_h:${start_width}:0" -an -sn $loop_option "${base_name}-split$((i+1)).$output_extension"
done
...
echo "File split into 5 vertical strips and hex changed for steam: ${base_name}-split1.$output_extension, ${base_name}-split2.$output_extension, ..., ${base_name}-split5.$output_extension"
